...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    console.log("Mail options defined for both emails");

    // Send both emails
    await transporter.sendMail(mailOptions1);
    console.log("First email sent to user:", data.email);

    await transporter.sendMail(mailOptions2, (error, info) => {
      console.log("Processing email sending to marketing...");

      if (error) {
        console.error("Error sending email:", error);
      } else {
        console.log("Email sent to marketing successfully:", info.response);
      }
      transporter.close();
    });
    ...
    return res.status(500).json({ errMessage: "Failed to send email", err: error.message });
  }
});
